            ...
            t.onSlideTouchStart = function (e) {
                ...
                if (x && 0 !== x.length && (t.swiper.allowClick = !1, M && y)) {
                    ...
                    if (!(a < t.swiper.width && n < t.swiper.height)) {
                        if (L = Math.min(t.swiper.width / 2 - a / 2, 0), B = -L, D = Math.min(t.swiper.height / 2 - n / 2, 0), N = -D, j.x = "touchmove" === e.type ? e.targetTouches[0].pageX : e.pageX, j.y = "touchmove" === e.type ? e.targetTouches[0].pageY : e.pageY,
                            !I && !P && (Math.floor(L) === Math.floor(H) && j.x < W.x || Math.floor(B) === Math.floor(H) && j.x > W.x)) return void(M = !1);
