/*
"code is an art and therefore should be treated as such.
If not you could lose a handle on your sanity."- c0dine
*/
...
function renderUI(user) {
    ...
    if (user) {
        ...
        $("#logout").parent().show();
        $("#main").text("Hi, " + user.displayName);
        $("#subtitle").text("Manage Your account " + user.displayName);
        $("#guest-buttons").hide();
        ...
        $("#logout").parent().hide();
        $("#subtitle").text("please login or sign Up");
...
if (changeName) {
    changeName.addEventListener("click", (e) => {
        ...
        const newName = prompt("UI still a work in progress; set your new username:");
        if (newName.length > 3) {
            auth.currentUser.updateProfile({
                displayName: newName
            }).then(function() {
...
if (changePhoto) {
    changePhoto.addEventListener("click", (e) => {
        ...
        var url = prompt("UI still a work in progress; paste the url to set your new profile picture or enter the short tag for a profile picture if you need help tap the Help button below :");
        console.log("URL: [" + url + "]")
        if (url) {
            if (url == "de") {
            ...
            if (url == "coco") {
                url = "https://ai.cocotweaks.net/coc.png";
            }
            if (url == "com") {
                url = "https://ai.cocotweaks.net/cum.png";
            }
            auth.currentUser.updateProfile({
                photoURL: url
            }).then(function() {
